...
void solve(const int& t){
    ...
    int common=gcd(a,b);//store only the greatest common divisor to get its divisors
                                //because these divisors are the common divisors for both numbers
    
    for(int i = common; i >= 0; i--){ //loop over all numbers and each time decrement the k by 1  (it is the same idea as incrementing a counter with 1 till meeting k)
        if(common%i==0){
            k--;
            if(k==0){
                cout<<i;
