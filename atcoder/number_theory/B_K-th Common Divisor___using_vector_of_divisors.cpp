//solution for K-th biggest (not smallest) Common Divisor problem
//link of the problem : https://atcoder.jp/contests/abc120/tasks/abc120_b
...
void solve(const int& t){
    ...
    int common=gcd(a,b);//store only the greatest common divisors to get its divisors
                                //because these divisors are the common divisors for both numbers
    
    vector<int>divisors;
    for(int i = 1; i*i <= common; i++){
        if(common%i==0){
            divisors.push_back(i);
            if(i!=common/i)divisors.push_back(common/i);
        }
    }
    sort(divisors.rbegin(),divisors.rend());
    cout<<divisors[k-1];//0-indexed;
    
    // //dbg:
    // for(auto&x:divisors)cerr<<x<<' ';
